resource "aws_s3_bucket" "tf-state" {
  bucket = "${var.org_name}-sharedservices-tf-state"
  acl    = "private"
  versioning {
    enabled = true
  }
  tags = {
    Name        = "${var.org_name}-sharedservices-tf-state"
  }
  ...
  "Statement": [
    {
      ...
      "Effect": "Allow",
      "Principal": {
        "AWS": "arn:aws:iam::${var.sharedservices_acct_id}:role/${var.org_name}-deploy-role"
      },
      "Action": "s3:*",
      "Resource": [
